...
namespace Guirao.UltimateTextDamage
{
    ...
    public class UTDWindow : EditorWindow
    {
        ...
        void OnGUI( )
        {
            GUILayout.Label( "Ultimate Text Damage v1.0.4" , EditorStyles.helpBox );
            ...
            if( newValue != useNewText )
            {
                ...
                {
                    AddDefineSymbol( );
                    EditorUtility.DisplayDialog( "UltimateTextDamage" , "You have switched to TextMeshPro. If you see errors in the console please be sure you have TextMeshPro installed through the package manager. Also be sure your UltimateTextDamage items use TextMeshPro, there are examples in the prefabs folder." , "OK" );
                }
                else
                {
                    RemoveDefineSymbols( );
